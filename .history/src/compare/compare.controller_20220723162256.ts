...
async downloadImages(@Body() link: any, @Response() reply: FastifyReply)
{
    ...
    console.log(link.0)
    //await this.compareService.downloadImage(link.links,"link.folderName")
    let testFolder = './images/Rednote11Gray1/'
    ...
    fs.readdir(testFolder, (err, files) => {
        files.forEach(file => {
          ...
          console.log(file);
          const buffer = fs.readFileSync(`./images/Rednote11Gray1/${file}`)
          reply.send(buffer)
