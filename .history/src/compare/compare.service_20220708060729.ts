...
export class CompareService {
                            ...
                            for (let product of productsArr.products) {
                                if (await this.doesExist(product.IvoryUid) == null) {
                                    ...
                                    if (kspProduct) {
                                        ...
                                        const productINDB = this.PriceCompare.findOne({ KspUid: product.KspUid })
                                        if((await productINDB).IvoryUid!=null)
                                        {
                                            ...
                                            const NewProduct = new this.PriceCompare(newProduct)
                                            NewProduct.save().catch(async (e) => {
                                                {
    ...
    async updateProduct(product: ObjectId, updates: any)
    {
        if(updates.KspUid)
        {
            const kspItem = this.PriceCompare.findOne({KspUid: updates.KspUid})
            const ivoryItem = this.PriceCompare.findById({_id: product})

            updates = {"IvoryUid": (await ivoryItem)._id, "IvoryPrice": (await ivoryItem).priceIvory}
            this.PriceCompare.findByIdAndDelete(product)
            product = (await kspItem)._id
