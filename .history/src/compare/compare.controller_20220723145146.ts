...
@Post('/images')
async downloadImages(@Body() link: any, @Response() reply: FastifyReply)
{

    console.log(link.link)
    await this.compareService.downloadImage(link.link,link.folderName)
    reply.type('application/octet-stream')
    reply.header('Content-Disposition', 'attachment; filename=' + '0.jpg');

reply.send(fs.readFileSync('./image/Rednote11Gray1./0.jpg'))
}
