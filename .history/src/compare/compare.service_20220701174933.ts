...
export class CompareService {
            ...
            await axios.get(`https://ksp.co.il/m_action/api/category/${cat}?sort=2&page=${page}`, {
                ...
                .then(async (res) => {
                    ...
                    for (let item of res.data.result.items) {
                        ...
                        //console.log( this.PriceCompare.findOneAndUpdate({KspUid: Number(item.uin), category: category}, {cat: cat}).exec())
    ...
    async loadCategory(category: string, brand: string) {
        ...
        for (let item of allItems) {
            ...
            console.log(item.KspUid)
            if (item.kspStockArray[item.kspStockArray.length-1].time.getDate() != date.getDate()
            ) {
                await this.wait(600)
    ...
    async getPendingApproval() {
        return this.PriceCompare.find({  IvoryUid: {$ne: 0}})
    }
    async getAllUins() {
                                ...
                                if (await this.doesExist(product.IvoryUid) == null) {
                                    ...
                                    if (kspProduct) {
                                        ...
                                        if (
                                            ...
                                            approvalStatus = "Approved by the system"

                                        }
                                        this.PriceCompare.findOneAndUpdate({ KspUid: product.KspUid }, { IvoryUid: product.IvoryUid, priceIvory: product.priceIvory, approvalStatus: approvalStatus }).exec()
                                        ...
                                        console.log(product.priceIvory)
                                        const Product = new this.PriceCompare(product);
                                        //console.log(JSON.stringify(Product))
                                        Product.save().catch(async (e) => {
                                            {
                                    ...
                                    this.PriceCompare.findOneAndUpdate({ IvoryUid: product.IvoryUid }, { priceIvory: product.priceIvory })
    ...
    async getKspUid(category: string, title: string, price: number) {
        ...
        const res = await this.PriceCompare.find({ category: category, $text: { $search: title } }).exec()
        if(res)
        {
        console.log(title.toUpperCase(), res[0].brand, res[0].title, console.log(res))
        await this.wait(5000)

        let resArr = res.filter((x) => !x.title.includes("מציאון")) //Making sure that the item is not regfurbished 
        if(res.filter((x) => title.toUpperCase().includes(x.brand.toUpperCase()))) //Making sure that one of the items in the array is from the same brand 
        {
         resArr = res.filter((x) => title.toUpperCase().includes(x.brand.toUpperCase()))
        ...
        if (res) {
            for (let item of resArr) {
                ...
                console.log()
                counter = 0;
                const itemArr = item.title.toUpperCase().split(" ");
    ...
    async getAllKspUins(): Promise<void> {
        ...
        for (let item of itemsWithoutKspUin) {
            ...
            if (kspItem != null) {
                ...
                this.PriceCompare.findOneAndUpdate({ IvoryUid: item.IvoryUid }, { $set: { KspUid: kspItem.uin, priceKsp: kspItem.price } }, { new: true }, (err, doc) => {
                    if (err) {
    ...
    {
        return await this.PriceCompare.updateMany({category: category, brand: brand},{inCharge: buyer})
    }
