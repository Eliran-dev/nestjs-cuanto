            ...
            await axios.get(`https://ksp.co.il/m_action/api/category/${cat}?sort=2&page=${page}`, {
                ...
                .then(async (res) => {
                    ...
                    .then(async (res) => {
                        ...
                        updatedItems = Object.values(res.data).filter((x: any) => x.discount != null)
                    ...
                    for (let item of res.data.result.items) {
                        ...
                        console.log(item.uin + " " + updatedItems.findIndex(x => x.uin == item.uin) == true + " " +  Number(item.price))
                        await this.wait(1000)
                        ...
                        product = {
                            ...
                            kspSql: (item.uinsql),
                            title: item.name,
                            priceKsp: Number(updatedItems[updatedItems.findIndex(x => x.uin == item.uin)].price) | Number(item.price),
                            category: GetCategoryFromKsp.category,
                            brand: item.brandName,
                            cat: cat
                        }
                        if(item.brandName == "Xiaomi" && item.uin == 206288)
                        {
                            console.log(item)
                        }
                        ...
                        },
                        {
                            ...
                            kspSql: (item.uinsql),
                            title: item.name,
                            priceKsp: updatedItems.findIndex(x => x.uin == item.uin) ? Number(updatedItems[updatedItems.findIndex(x => x.uin == item.uin)].price)  :Number(item.price),
                            category: GetCategoryFromKsp.category,
                            brand: item.brandName,
                            cat: cat
                        },
