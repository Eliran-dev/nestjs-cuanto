...
export class CompareService {
    ...
    async getPendingApproval() {
        return this.PriceCompare.find({ approvalStatus: "Pending"})
    }
    async getAllUins() {
                    ...
                    .then(async (res) => {
                        ...
                        for (let productsArr of res) {
                            for (let product of productsArr.products) {
                                if (await this.doesExist(product.IvoryUid) == null && (product.title).includes("Xiaomi")) {
                                    ...
                                    const kspProduct = await this.(route.category, product.title, product.priceIvory);
    ...
    async getKspUid(category: string, title: string, price: number) {
        ...
        let resArr = res.filter((x) => title.toUpperCase().includes(x.brand.toUpperCase()) && !x.title.includes("מציאון"))
