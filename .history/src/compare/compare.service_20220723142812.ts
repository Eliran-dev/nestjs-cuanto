...
this.wait(3000)
    ...
    async downloadImage(urls: Array<string>, folderName: string) {
        ...
        fs.mkdirSync(`./images/${folderName}/fixed`,{recursive: true} )
        ...
        await sharp(res.data).resize(500,428).toFile(`./images/${folderName}/fixed/${i}.jpg`)
     ...
     async resizeImages(urls: Array<string>, folderName: string)
     {
        for(let i=0; i < urls.length; i++)
        {
            sharp(`${folderName}/${i}.jpg`).resize(500,400).toFile(`./${folderName}/fixed/${i}.jpg`)
