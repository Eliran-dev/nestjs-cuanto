...
@Post('/images')
 downloadImages(@Body() link: any, @Response() reply: FastifyReply)
{

    console.log(link)
     this.compareService.downloadImage(link.links,link.folderName)
    console.log("link")

    reply.code(200).send("Successfully resized images")
}

@Post('/images/social')
 downloadImagesSocial(@Body() link: any, @Response() reply: FastifyReply)
{

    console.log(link)
     this.compareService.downloadImageSocial(link.link,link.folderName)
    console.log("link")
...
@Get('/images/:sku')
getImagesCounter(@Param('sku') sku: string, @Response() reply: FastifyReply)
{
    console.log(sku)
    fs.readdir(`./images/${sku}`, (err, files) => {
        reply.code(200).send(files.length)
        
      }); 

}
@Get('/images/:sku/:imageNumber')
getImage(@Param('sku') sku: string,@Param('imageNumber') imageNumber:number, @Response() reply: FastifyReply)
{
    const buffer = fs.readFileSync(`./images/${sku}/${imageNumber}.jpg`)
    reply.header(
        'Content-Disposition',
